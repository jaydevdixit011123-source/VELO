...
// Local fallback when no Groq key (works 100% offline & free)
function localReply(text) {
  const t = text.toLowerCase();
  if (t.includes('hello') || t.includes('hi') || t.includes('namaste') || t.includes('hii') || t.includes('hey'))
    return 'Hello Jaydev! 👋 How can I help you today?';
  if (t.includes('how are you')) return "I'm doing great, thanks for asking! 😊";
  if (t.includes('your name')) return "I'm VELO, your personal AI assistant, built to be fast, free and all yours.";
  ...
  if (t.includes('thanks') || t.includes('thank you') || t.includes('shukriya')) return "You're most welcome! Anytime. 🙌";
  if (t.includes('bye')) return "Goodbye! See you soon. 👋";
  return "I'm running in offline local mode, so I'm replying with built-in responses. Add a FREE Groq API key in Settings to unlock full AI chat. For now try: 'open notepad', 'show my notes', 'create reminder', 'screenshot', or 'what can you do'.";
}
...
function openApp(name) {
  ...
  const map = {
    'notepad': 'notepad', 'chrome': 'chrome', 'vscode': 'code', 'code': 'code',
    'calculator': 'calc', 'paint': 'mspaint', 'explorer': 'explorer', 'cmd': 'cmd',
    'terminal': 'cmd', 'task manager': 'taskmgr', 'control panel': 'control'
  };
  const target = map[n] || n;
  exec('start "" ' + target, { windowsHide: true }, (e) => {});
  return 'Opening ' + name + '...';
}
...
function setupIPC() {
  ...
  ipcMain.handle('ai:chat', async (_e, msg, history = []) => {
    const cfg = getConfig();
    const langHint = cfg.language === 'hinglish' ? 'Reply in Hinglish (Hindi + English mix), friendly and helpful.' :
      cfg.language === 'hindi' ? 'Reply in Hindi.' : 'Reply in English.';
    const memories = getMemory().slice(-5).map(m => m.text).join('; ');
    try {
      if (!cfg.groqKey) return { ok: true, response: localReply(msg), local: true };
      const messages = [
        { role: 'system', content: 'You are VELO, a helpful personal AI desktop assistant. ' + langHint + ' Keep answers clear and concise.' + (memories ? ' Remembered context: ' + memories : '') },
        ...(history || []).slice(-12),
  ...
  ipcMain.handle('browser:open', (_e, url) => {
    let u = url;
    if (!/^https?:\/\//i.test(u)) u = 'https://www.google.com/search?q=' + encodeURIComponent(url);
...
function createWindow() {
  mainWindow = new BrowserWindow({
    ...
    backgroundColor: '#0b0f1a', title: 'VELO - Personal AI Assistant',
